...
Gli agenti descritti in tuProlog sono entit\`a computazionali autonome che effettuano delle operazioni che impattano sul proprio stato e su quello dell'ambiente in cui sono immersi. La reattivit\`a \`e una caratteristica importante per quanto riguada gli agenti poich\`e devono essere pronti a reagire ad eventi o cambiamenti che avvengono nello spazio.
Un'agente sar\`a quindi definito attraverso una teoria composta da predicati, i quali sono i piani dell'agente. I piani saranno composti di una serie di operazioni volte a svolgere un'azione.
...
L'agente Gretel adotta invece un movimento che genera in modo casuale direzione e velocit\`a del nodo secondo la distribuzione di L\`evy.
...
Ad ogni passo cerca una briciola negli spazi di tuple presenti nel suo vicinato e se la trova modifica la direzione verso il nodo che contiene la briciola.
...
\end{table}
Su 17 simulazioni eseguite, con la configurazone del Codice sorgente \ref{lst:SimulazioneBreadcrumbs} e con le teorie dei due agenti descritte sopra, \`e risultato che l'agente Hansel ha raggiunto Gretel 13 volte con un tempo medio di 334 tau (tempo della simulazione).
Le simulazioni non valide sono contraddistinte dal risulatato KO. In queste simulazioni l'agente Gretel nel suo spostamento ha superato i limiti formati dagli spazi di tuple presenti nello spazio.
