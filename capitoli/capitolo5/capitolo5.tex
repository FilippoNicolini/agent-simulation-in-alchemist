...
\hspace*{-2.5cm}
\includegraphics[width=19.5cm]{images/UML_agenti.png} % inserisce una figura larga 12.5cm
...
Per quanto riguarda le frecce, quelle con la linea tratteggiata stanno ad indicare la relazione `\textit{implements}', ovvero che una certa classe implementa l'interfaccia indicata, mentre quelle con il tratto continuo significano `\textit{extends}', ovvero che una classe estende le proprietà e le funzionalità di quella da cui deriva.
...
\begin{itemize}
\item[1.] TimeDistribution, tramite invocazione del metodo `\texttt{crea\-teTimeDis\-tri\-bu\-tion(rand, env, node, par)}' implementato nella classe AgentIncarnation;
\item[2.] AgentReaction;
\item[3.] MovementAgent;
\item[4.] Condition, tramite invocazione del metodo `\texttt{cre\-ateCon\-di\-tion(rand, env, node, td, react, par)}' implementato nella classe AgentIncarnation.
\end{itemize}
...
Inoltre, è implementato il metodo `\texttt{updateAgentsPosition()}' che innesca l'aggiornamento dei fatti, quali posizione e distanza dagli altri agenti, per ogni agente all'interno del nodo. Secondo l'implementazione attuale, questo metodo viene invocato solamente dall'agente `Movement', inserito alla creazione del nodo, che, nel suo ciclo di ragionamento, ha come unico compito quello di invocare questo metodo per scatenare l'aggiornamento. Gli agenti quindi verranno aggiornati quando lo scheduler di Alchemist farà eseguire il ciclo di ragionamento dell'agente `Movement'.
...
Non sono quindi implementati i metodi `\texttt{execute()}', `\texttt{cloneAction(node, re\-ac\-tion)}', definiti dall'interfaccia Action, dalla cui implementazione è caratterizzato il comportamento dell'agente. In particolare, il metodo `\texttt{execute()}', che è quello che rappresenta il ciclo di ragionamento, è invocato periodicamente dal core di Alchemist (questo perchè le Condizioni collegate alla stessa Reazione sono impostate per essere sempre vere) per lo svolgimento delle azioni.
...
Per inizializzare un agente sono state definite una serie di operazioni posizionate poi all'interno dei metodi `\texttt{initializeAgent()}' e `\texttt{initReasoing()}'.
...
Per quanto riguarda il metodo per l'invio della richiesta, l'agente recupera i fatti presenti all'interno della teoria e per ognuno costruisce una richiesta formata da un termine, l'istanza dell'agente e la primitiva(`\texttt{in}', `\texttt{rd}', `\texttt{out}') che esegue quel termine. Come già descritto, la scelta implementativa è stata quella di inserire la richiesta di scrittura sullo spazio di tuple più vicino, mentre quelle delle altre primitive su tutti gli spazi di tuple del vicinato.

Il metodo che consente all'agente di ricevere il risultato della richiesta è stato implementato fornendo in ingresso solamente un parametro, il termine recuperato dallo spazio di tuple. In questo modo all'interno della teoria dell'agente sarà sufficiente trovare una regola `\texttt{onResponseMessage(T)}', dove \texttt{T} è il parametro ricevuto, idonea al contesto per poi creare una nuova intenzione da inserire tra quelle da eseguire.

\subsection{Dettaglio implementazioni}\label{sctn:DettaglioImplementazioni}
In questa sezione verranno descritti alcuni dettagli relativi all'implementazione realizzata quali il funzionamento dell'agente `Postman', la referenziazione e l'utilizzo di oggetti Java in tuProlog, la creazione delle intenzioni.
...
Nel Codice sorgente \ref{lst:BindingOggettiJavaIntuProlog} è mostrato come sono state configurate le due variabili `\texttt{agent}' e `\texttt{node}' disponibili nella teoria tuProlog e che si riferiscono alle istanze di agente e nodo presenti nel simulatore Alchemist.
...
È stata poi utilizzata la funzione `\texttt{register(id, obj)}' messa a disposizione dalla libreria che permette di registrare al suo interno identificativi di variabili utilizzabili in tuProlog riferiti ad oggetti Java.
Come si può vedere nel Codice sorgente \ref{lst:BindingOggettiJavaIntuProlog} recuperato dalla classe dell'agente, all'interno della libreria viene prima registrata la variabile `\texttt{agent}' referenziata alla classe stessa e poi la variabile `\texttt{node}' riferita al nodo, recuperato attraverso il riferimento dell'agente.
...
Nella prima riga viene fatta un'invocazione di una funzione `\texttt{doSomething()}' che non ha parametri in ingresso.
Nel secondo esempio l'invocazione riguarda la funzione `\texttt{doSome\-thingWithParams(string, term)}' che riceve parametri di tipo String e Term. La signature del metodo Java è fondamentale: se il tipo degli oggetti attesi non corrisponde a quelli passati il metodo risulterà non invocato.
...
La procedura per creare un'intenzione inizia dalla definizione della struttura del template utilizzata per cercare il piano adatto per reagire a quell'evento. Invocando la funzione `\texttt{createPlanStruct(term)}', dove \texttt{tem} è l'evento per il quale si sta creando l'intenzione, viene restituito un oggetto strutturato come mostrato nel Codice sorgente \ref{lst:OggettoStrutturaPiano}.
...
L'oggetto \texttt{struct} che è stato creato viene poi passato all'invocazione del metodo `\texttt{createIntention(term, errMsg)}', dove \texttt{errMsg} è una stringa contenente un messaggio utilizzato in caso di errore.
...
Ottenuta la lista di azioni \texttt{body} si inizia la vera e propria creazione dell'azione: per prima cosa viene generato un identificativo univoco \texttt{id} e successivamente viene creata la struttura `\texttt{intention(id,body)}' che poi è inserita all'interno della teoria dell'agente. Se l'inserimento è completato con successo l'\texttt{id} è inserito anche nella lista lato Java che mantiene gli identificativi di tutte le intenzioni presenti nella teoria dell'agente, e che è utilizzata per gestire l'esecuzione delle intenzioni con metodo Round-Robin.
...
Alla base vi è sicuramente la possibilità di ospitare codice sorgente all'interno di vari repository che possono essere pubblici, privati o open source con la possibilità di versionare ogni singola modifica e tenendo traccia di chi l'abbia prodotta \footnotemark.
...
\footnotetext{GitHub: \url{https://github.com}}
...
\footnotetext{TravisCI: \url{https://travis-ci.org}}
...
La sezione `\texttt{incarnation}' è obbligatoria. Il parser YAML si aspetta una stringa che rappresenta il nome dell'incarnazione da utilizzare per la simulazione.
\medskip
...
Utilizzando la keyword `\texttt{environment}' si può scegliere quale definizione di ambiente utilizzare per la simulazione.
\medskip
...
I collegamenti tra i nodi che verranno utilizzati nella simulazione sono specificati nella sezione `\texttt{network-model}'. Un esempio per la costruzione di collegamenti è il seguente.
\medskip
...
Il posizionamento dei nodi viene gestito dalla sezione `\texttt{displacements}'. Questa sezione può contenere uno o più definizioni di disposizioni per i nodi.
Il parametro `\texttt{in}' definisce la geometria all'interno della quale verranno disposti i nodi, utilizzando ad esempio punti o figure come cerchi o rettangoli, mentre il parametro `\texttt{programs}' definisce le reazioni da associare ad ogni nodo di quella certa disposizione.

Esempi di classi utilizzabili nel parametro `\texttt{in}' sono Point e Circle.
...
Il parametro `\texttt{programs}' rappresenta le reazioni da associare ai nodi ed accetta una lista di reazioni, le quali a loro volta sono formate da una lista di parametri. Un esempio di definizione di una reazione è utilizzando `\texttt{time-distribution}' (valore utilizzato per settare la frequenza) e `\texttt{program}' (parametro che viene passato alla creazione della reazione e che può essere utilizzato per istanziare condizioni e azioni).
