...
Il concetto di vicinato viene gestito da Alchemist attraverso la funzione di linking-rule, che utilizza il parametro `network-model' se opportunamente impostato in fase di configurazione della simulazione. Lato implementativo è sufficiente richiedere all'Environment di fornire tutti i nodi presenti nel vicinato con i quali è poi possibile effettuare altre operazioni come, ad esempio, ottenere la distanza tra i vari nodi e recuperare gli agenti presenti in ogni nodo, utilizzando l'apposita mappa.
\\
...
%TODO - rimuovere se si toglie la sotto-sezione nel Dettaglio implementazioni
...
In questa sezione verranno descritti alcuni dettagli implementativi dell'implementazione realizzata, quali:
\begin{itemize}
\item funzionamento dell'agnete `Postman';
\item utilizzo di oggetti Java in tuProlog;
\item interrogazioni della teoria dell'agente tramite Alchemist.
\end{itemize}
...
\textbf{TODO - descrivere come si crea un'intenzione (prima completare il codice) [recupero body e creazione intenzione]}
...
Alla base vi è sicuramente la possibilità di ospitare codice sorgente all'interno di vari repository che possono essere pubblici, privati o open source con la possibilità di versionare ogni singola modifica e tenendo traccia di chi l'abbia prodotta.
...
I parametri esterni si riferiscono alla qualità del software così come è percepita dai suoi utenti e includono correttezza, affidabilità, robustezza, efficienza, usabilità, eco-compatibilità. %TODO descrivere velocemente le caratteristiche
I parametri interni si riferiscono alla qualità del codice del software così come è percepita dagli sviluppatori e includono verificabilità, manutenibilità, riparabilità, evolvibilità, riusabilità, portabilità, leggibilità, modularità. %TODO descrivere velocemente le caratteristiche

Le metriche software più utilizzate sono:
\begin{itemize}
