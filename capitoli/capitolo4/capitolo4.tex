...
\begin{itemize}
\item controllare se la richiesta è andata a successo;% (funzione \texttt{isSuccess});
\item recuperare il valore di una singola variabile inserita nel template;% (funzione \texttt{getTerm});
\item ottenere la soluzione del template, ovvero dove tutte le variabili utilizzate sono sostituite con il valore del fatto o della regola ricavato;% (funzione \texttt{getSolution});
\item verificare se ci fossero altre possibili alternative che corrispondono al template utilizzato;% (funzione \texttt{hasOpenAlternatives});
\end{itemize}
